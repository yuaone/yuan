...
// ─── L0: Milestone (Multi-Session Goal Tracking) ───

/** L0: Milestone — spans multiple sessions to track a high-level, long-running goal */
export interface Milestone {
  ...
  /** L1 strategic goal IDs this milestone encompasses */
  subGoals: string[];
...
export interface RePlanTrigger {
  ...
  type: "error" | "new_info" | "dependency_change" | "user_feedback" | "context_overflow" | "strategic";
  /** Human-readable description */
  description: string;
...
async planStrategic(
  ...
  async replan(
    ...
    // Strategic trigger: replace the entire strategic plan
    if (trigger.type === "strategic") {
      await this.replaceStrategicGoal(currentPlan, trigger.description, llmClient);
      return {
        strategy: "full_replan",
        modifiedTasks: currentPlan.tactical,
        reason: `Strategic replan: ${trigger.description}`,
      };
    }
  ...
  /**
   * Create a new milestone for multi-session goal tracking.
   *
   * @param title - Human-readable title
   * @param description - Detailed description
   * @param subGoals - L1 strategic goal IDs this milestone encompasses
   * @returns The created Milestone
   */
  createMilestone(title: string, description: string, subGoals: string[]): Milestone {
  ...
  /**
   * Update the status of an existing milestone.
   *
  ...
  /**
   * Get the currently active milestone (first one with "in_progress" status).
   *
   * @returns The active milestone, or null if none
   */
  ...
  /**
   * Auto-create a milestone from a hierarchical plan.
   * Uses the plan's goal as the title and strategic sub-goals as sub-goal IDs.
   *
   * @param plan - The hierarchical plan to derive a milestone from
   * @returns The created Milestone
   */
  milestoneFromPlan(plan: HierarchicalPlan): Milestone {
    const subGoalIds = plan.strategic.subGoals.map((sg) => sg.id);
    const estimatedSessions = Math.max(1, Math.ceil(plan.totalEstimatedIterations / 20));
    const milestone = this.createMilestone(plan.goal, plan.strategic.description, subGoalIds);
    milestone.targetSessions = estimatedSessions;
    milestone.updatedAt = new Date().toISOString();
    return milestone;
  }
  ...
  /**
   * Replace the entire strategic goal in a plan with a new one generated by the LLM.
   * Useful when the current strategic direction needs a fundamental change.
   *
   * @param plan - The current hierarchical plan (mutated in place)
   * @param newGoal - The new high-level goal description
   * @param llmClient - BYOK LLM client for strategic re-planning
   * @returns The updated plan with new strategic goal, tactical tasks, and metadata
   */
  async replaceStrategicGoal(
    plan: HierarchicalPlan,
    newGoal: string,
    llmClient: BYOKClient,
  ): Promise<HierarchicalPlan> {
    // Re-run L1 strategic planning with the new goal
    const newStrategic = await this.planStrategic(newGoal, llmClient);

    // Re-run L2 tactical planning
    const newTactical = await this.planTactical(newStrategic, llmClient);

    // Re-run L3 operational planning if enabled
    const newOperational = new Map<string, OperationalAction[]>();
    if (this.config.enableOperationalPrePlanning) {
      for (const task of newTactical) {
        const actions = await this.planOperational(task, llmClient);
        newOperational.set(task.id, actions);
      }
    }

    // Update plan in place
    plan.goal = newGoal;
    plan.strategic = newStrategic;
    plan.tactical = newTactical;
    plan.operational = newOperational;
    plan.parallelizableGroups = this.findParallelGroups(newTactical);
    plan.criticalPath = this.findCriticalPath(newTactical);
    plan.totalEstimatedIterations = newTactical.reduce(
      (sum, t) => sum + t.estimatedIterations,
      0,
    );
    plan.estimatedTokenBudget = this.estimateTokenBudget(plan);
