...
export class ImpactAnalyzer {
  ...
  async analyzeChanges(changedFiles: string[]): Promise<ImpactReport> {
    try {
      const [affectedFiles, affectedTests, affectedAPIs, breakingChanges] =
        await Promise.all([
    ...
    try {
      ...
      while (queue.length > 0) {
        ...
        if (depth < this.maxDepth) {
          ...
          for (const pf of projectFiles) {
            if (seen.has(pf) || pf === file) continue;
            const pfContent = await readFileSafe(pf);
            if (!pfContent) continue;
      ...
      if (this.includeTests) {
        for (const pf of projectFiles) {
          if (seen.has(pf) || !TEST_FILE_PATTERNS.some((p) => p.test(pf))) continue;
          const pfContent = await readFileSafe(pf);
          if (!pfContent) continue;
